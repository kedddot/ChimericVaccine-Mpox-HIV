...
def _reference_sequence(target):
    ...
    CORRECTED 2026-08-30, same defect as Phase1De_foldPrep.subunit_sequence().

    This used to slice every HIV target unconditionally with the fixed offsets
    in HIV_SUBUNIT_RANGES, because Phase 1A stored raw POLYPROTEINS (gp120/gp41
    shared one 856-aa Env record; p17/p24 shared one 1437-aa Gag-Pol record) and
    the mature chain had to be cut out here.

    Phase 1A now performs that slicing itself, so its Var_01 files ARE mature
    subunits. Slicing again would cut a subunit out of a subunit -- on the
    current output that yields gp41 = 0 residues and p24 = 96 of 231. The
    reference sequence is what every epitope offset and every SEMA per-residue
    score is measured against, so a wrong reference would silently mislabel
    every B-cell corroboration call for that antigen.

    The record is now slice-CHECKED rather than assumed: if its length already
    falls inside the canonical mature band it is used as-is, and a slice landing
    outside that band raises instead of returning a wrong reference.
    """
    ...
    if band and band[0] <= len(seq) <= band[1]:
        return seq                      # already the mature subunit from 1A

    sliced = seq[info["start"] - 1:info["end"]]
    if band and not (band[0] <= len(sliced) <= band[1]):
        raise ValueError(
            f"{target}: slicing the Var_01 record ({len(seq)} aa) at "
            f"[{info['start']}:{info['end']}] gives {len(sliced)} aa, outside the "
            f"canonical band {band[0]}-{band[1]}. Refusing to use a reference "
            "sequence that is not the intended mature subunit.")
    return sliced
