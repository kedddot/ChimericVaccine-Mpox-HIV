...
def subunit_sequence(target):
    """
    Returns (sequence_to_fold, source_description).

    CORRECTED 2026-08-30. This function used to slice every HIV target with the
    fixed offsets in HIV_SUBUNIT_RANGES, because Phase 1A stored raw
    POLYPROTEIN records and the mature subunit had to be cut out here.

    Phase 1A now performs that slicing itself, so its files ARE mature subunits.
    Applying the offsets a second time would cut a subunit out of a subunit.
    Measured on the current Phase 1A output, the old code would have produced:
        HIV_gp120  480 aa file -> slice [32:504]  -> 449 aa  (31 residues lost)
        HIV_gp41   349 aa file -> slice [505:856] ->   0 aa  (EMPTY)
        HIV_p17    131 aa file -> slice [2:135]   -> 130 aa
        HIV_p24    231 aa file -> slice [136:366] ->  96 aa  (135 residues lost)
    Those FASTAs would have been submitted to AlphaFold Server, queued, and
    returned as structures of the wrong molecule -- silently, since nothing
    downstream re-checks the folded sequence's length.

    The record is now slice-checked rather than assumed: if its length already
    falls inside the canonical mature band for that subunit, it is used as-is;
    only a record long enough to still be a polyprotein is cut.
    """
    ...
    band = common.HIV_SUBUNIT_LENGTH_BAND.get(target)
    if band and band[0] <= len(parent_seq) <= band[1]:
        return parent_seq, (f"{fname} (already a mature {target} subunit from "
                            f"Phase 1A, {len(parent_seq)} aa -- not re-sliced)")

    s, e = info["start"], info["end"]
    sliced = parent_seq[s - 1:e]
    if band and not (band[0] <= len(sliced) <= band[1]):
        raise ValueError(
            f"{target}: slicing {fname} ({len(parent_seq)} aa) at [{s}:{e}] gives "
            f"{len(sliced)} aa, outside the canonical band {band[0]}-{band[1]}. "
            "Refusing to emit a fold submission for a sequence that is not the "
            "intended subunit.")
    return sliced, f"{fname} residues {s}-{e} ({info['parent']} subunit)"
...
def run_fold_prep():
    ...
    paste_dir = os.path.join(sub_dir, "_paste_ready")
    ...
    for target in TARGETS:
        ...
        # PASTE-READY PLAIN SEQUENCE. AlphaFold Server, PSIPRED, NetSurfP,
        # CamSol, AllerTOP and AllergenFP all take a raw amino-acid string typed
        # into a box -- they do not accept a FASTA header, and some mangle or
        # silently truncate a wrapped multi-line body. Emitting both formats
        # means a manual gate cannot fail on file format, which is a real
        # failure mode when a job is queued for hours before anyone notices it
        # was rejected.
        with open(os.path.join(paste_dir, f"{target}.txt"), "w") as f:
            f.write(seq + "\n")
    ...
    print(f"[INFO] Paste-ready plain sequences (no header, one line) in: {paste_dir}")
    print("       Use these for AlphaFold Server / PSIPRED / NetSurfP / CamSol /")
    print("       AllerTOP / AllergenFP -- they take a raw sequence, not a file.")
