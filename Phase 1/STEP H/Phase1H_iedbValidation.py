...
# =============================================================================
# FORCE IPv4 FOR IEDB REQUESTS -- MEASURED 100x SPEEDUP, NOT A GUESS.
#
# On this network, getaddrinfo returns a NAT64-synthesised IPv6 address
# (64:ff9b::/96) for query-api.iedb.org ahead of the real IPv4 one. The IPv6
# SYN is never answered, so every request sits in SYN_SENT until the OS gives
# up and falls back to IPv4. requests/urllib3 has no Happy Eyeballs, so the
# stall is paid IN FULL ON EVERY PAGE.
#
# MEASURED, same query, same session:
#     as-is        60.7 s   HTTP 200, 50 rows
#     IPv4-forced   0.6 s   HTTP 200, 50 rows
# Vaccinia alone is 17,903 epitopes = 18 pages at PAGE=1000, so the stall cost
# ~18 minutes of pure waiting per run and the step looked hung at 0% CPU --
# the same false "slow work" signature already documented for Entrez in the
# project traps list.
#
# This changes NOTHING about the data: same host, same endpoint, same response
# bytes. It only stops the resolver offering an address that cannot connect.
# Restricted to this module's requests; nothing else in the pipeline is
# affected.
# =============================================================================
...
def _ipv4_only_getaddrinfo(*args, **kwargs):
    ...
    # Fall back to whatever was offered if there is no IPv4 record at all,
    # rather than turning a working IPv6-only host into a hard failure.
    return v4 or infos
