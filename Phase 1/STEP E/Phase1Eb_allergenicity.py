...
def run_step1eb_allergenicity():
    ...
    # Every manual web gate in this pipeline must also emit a PASTE-READY plain
    # file: AllerTOP, AllergenFP, CamSol, PSIPRED and NetSurfP all take a raw
    # sequence typed into a box and reject or mangle a pasted FASTA header.
    paste_export = os.path.join(output_base, "Allergenicity_Query_PASTE.txt")
    # AllergenFP's ACC fingerprint cannot be computed below 16 aa (deviation
    # #6), so it gets its OWN file holding only the peptides it can score.
    # Sending it the full pool would make the run fail or, worse, return a
    # short result set that silently mis-aligns with the query order.
    fp_fasta_export = os.path.join(output_base, "AllergenFP_Query_16aa.fasta")
    fp_paste_export = os.path.join(output_base, "AllergenFP_Query_16aa_PASTE.txt")
    # Pep_N ids are anonymous, so keep a map back to antigen/class/variant.
    # Results are re-joined BY SEQUENCE, not by id, so this is for audit and
    # for reading a returned result file by eye -- not a load-bearing key.
    map_export = os.path.join(output_base, "Allergenicity_Query_MAP.csv")
    ...
    if new_peptides:
        ...
        with open(fasta_export, "w") as f_fasta, open(paste_export, "w") as f_paste:
            for i, pep in enumerate(new_peptides):
                f_fasta.write(f">Pep_{i}\n{pep}\n")
                f_paste.write(f"{pep}\n")

        fp_peptides = [p for p in new_peptides if len(p) >= 16]
        with open(fp_fasta_export, "w") as f_fa, open(fp_paste_export, "w") as f_pa:
            for i, pep in enumerate(new_peptides):
                if len(pep) < 16:
                    continue
                # Keep the SAME Pep_N index as the AllerTOP file so a peptide
                # carries one id across both submissions.
                f_fa.write(f">Pep_{i}\n{pep}\n")
                f_pa.write(f"{pep}\n")

        pep_meta = {}
        for row in rows:
            pep_meta.setdefault(row['Peptide'], row)
        with open(map_export, "w", newline='') as f_map:
            w = csv.writer(f_map)
            w.writerow(["Query_ID", "Peptide", "Length", "Target", "Epitope_Class",
                        "Variant", "Sent_To_AllerTOP", "Sent_To_AllergenFP"])
            for i, pep in enumerate(new_peptides):
                m = pep_meta.get(pep, {})
                w.writerow([f"Pep_{i}", pep, len(pep), m.get("Target", ""),
                            m.get("Type", ""), m.get("Variant", ""),
                            "YES", "YES" if len(pep) >= 16 else "NO (<16 aa)"])
        ...
        print(f"1. AllerTOP batch  ({len(new_peptides):3d} seqs): {os.path.basename(fasta_export)}")
        print(f"                                  paste-ready: {os.path.basename(paste_export)}")
        print(f"2. AllergenFP batch ({len(fp_peptides):3d} seqs): {os.path.basename(fp_fasta_export)}")
        print(f"                                  paste-ready: {os.path.basename(fp_paste_export)}")
        print(f"   ({n_too_short} new peptides are below 16 aa -- AllergenFP cannot score them")
        print(f"    (deviation #6); they are pre-filled N/A and excluded from its file.)")
        print(f"3. Query map (id -> antigen/class/variant): {os.path.basename(map_export)}")
        print(f"4. Fill in the blank rows in: {os.path.basename(manual_results_file)} -- already-answered rows are untouched")
        print(f"5. Re-run this script to execute the filtering matrix.\n")
        sys.exit(0)

    fieldnames = original_fields + ["QN_Ratio", "Surface_Charge", "Surface_Charge_Deprioritized",
                                     "Allergen_Homology", "Allergen_Homology_Kmer",
    ...
    for i, row in enumerate(rows):
        ...
        # -------------------------------------------------------------------
        # FAO/WHO CONTIGUOUS-IDENTITY SCREEN -- added 2026-08-31, deviation #27.
        #
        # Does this peptide share an identical stretch with a KNOWN allergen?
        # A factual lookup, not a prediction, so unlike AllerTOP/AllergenFP/
        # AllerCatPro it has NO minimum length and works on a 9-mer -- measured:
        # it detects a 9-aa Bet v 1 fragment that AllerCatPro reports as "no
        # significant hit" because a query that short cannot reach BLAST
        # significance.
        #
        # A HIT EXCLUDES, on the same principle as deviation #1's toxicity rule:
        # real homology evidence, not a score. A miss is NOT proof of safety --
        # the reference holds 1,020 reviewed allergens, novel allergens are in
        # no database, and only exact matches are found. It is recorded, never
        # reported as a clean negative.
        # -------------------------------------------------------------------
        try:
            homology_hit, homology_kmer = common.allergen_homology(pep)
        except FileNotFoundError as exc:
            homology_hit, homology_kmer = False, None
            if "allergen_ref_warned" not in globals():
                globals()["allergen_ref_warned"] = True
                print(f"[WARNING] allergen homology screen skipped: {exc}")

        is_allergen = fail_qn or fail_tools or homology_hit
        ...
        clean_row.update({
            ...
            "Allergen_Homology": "HIT" if homology_hit else "NONE",
            "Allergen_Homology_Kmer": homology_kmer or "",
        ...
        if is_unresolved:
            ...
            if homology_hit:
                reasons.append(f"ALLERGEN_HOMOLOGY (exact {len(homology_kmer)}-mer "
                               f"'{homology_kmer}' shared with a curated allergen)")
