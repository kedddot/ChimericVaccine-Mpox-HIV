...
def _parse_netsurfp_csv(path):
    """
    Parses NetSurfP-3.0's per-residue export.
    ...
    REWRITTEN 2026-08-31 AGAINST A REAL DOWNLOAD -- the format this
    function originally targeted (a comma-delimited CSV with a `q3`
    letter column) does not match what the live NetSurfP-3.0 web service
    actually hands back. The real download is:
      - TAB-delimited, not comma
      - NO header row of column NAMES at all -- only a `#`-comment block
        describing each column by POSITION
      - NO discrete secondary-structure letter column. Column 1 (0-indexed)
        is a burial call (B/E, buried/exposed -- solvent accessibility,
        NOT secondary structure), and the only secondary-structure signal
        is three raw PER-RESIDUE PROBABILITIES: P(helix), P(strand),
        P(coil) at columns 7, 8, 9.
    So a discrete H/E/C call per residue does not exist in the file -- it
    is DERIVED here as argmax(P_helix, P_strand, P_coil), the standard way
    to turn a 3-way softmax into a hard class call, and the same principle
    NetSurfP's own web viewer uses to render its on-screen prediction.
    VERIFIED against the actual 2026-08-31 download for this construct:
    570 data rows parsed from a 570 aa sequence, exact match.

    The original comma/q3-column parser is kept as a fallback and tried
    first, in case a future NetSurfP release (or a different tool
    substituted later) really does ship a q3 column -- this only falls
    through to the real-format parser when that attempt finds nothing.
    """
    import csv as _csv

    def _try_csv_q3(path):
        ...
        for r in rows:
            ...
            if ss in counts:
                counts[ss] += 1
        total = sum(counts.values())
        return counts if total else None

    def _parse_probability_tsv(path):
        """The real NetSurfP-3.0 download: tab-delimited, `#`-commented
        header, no column names, class derived from argmax(P_H, P_E, P_C)
        at columns 7/8/9 (0-indexed)."""
        counts = {"H": 0, "E": 0, "C": 0}
        with open(path) as f:
            for line in f:
                line = line.rstrip("\n")
                ...
                if len(cols) < 10:
                    continue
                try:
                    p_helix, p_strand, p_coil = float(cols[7]), float(cols[8]), float(cols[9])
                except ValueError:
                    continue
                call = max(("H", p_helix), ("E", p_strand), ("C", p_coil), key=lambda x: x[1])[0]
                counts[call] += 1
        return counts if sum(counts.values()) else None

    # _try_csv_q3 is only a probe for an older/different NetSurfP export
    # shape. Run against a real download, csv.DictReader (comma-delimited)
    # misreads the tab-delimited data and, worse, a citation line in the
    # file's own comment block ("...Volume 50, Issue W1...") contains real
    # commas, so some rows get MORE fields than the header and DictReader
    # stuffs the overflow into a list -- crashing a naive .strip() call.
    # Any failure here just means "not the old format": fall through to the
    # real parser rather than let a malformed probe abort the import.
    try:
        counts = _try_csv_q3(path)
    except Exception:
        counts = None
    counts = counts or _parse_probability_tsv(path)
    if counts is None:
        raise ValueError(
            f"Could not parse a secondary-structure signal from {path} under "
            f"either the comma/q3-column format or the real tab-delimited "
            f"probability format. Verify against your actual download."
        )
    total = sum(counts.values())
    return {
        "helix_fraction": counts["H"] / total,
